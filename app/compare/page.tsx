...
import { COMPARISON_DEMO_DATA, ALL_DEMO_DOCUMENTS } from '@/lib/mockData';
import {
  GitCompare,
  Sparkles,
  ArrowRight,
...
export default function ComparePage() {
  const { currentDocument, documentsList } = useDocument();

  const [selectedFilter, setSelectedFilter] = useState<string>('All');
  const [docASelection, setDocASelection] = useState<string>('draft-a');
  const [docBSelection, setDocBSelection] = useState<string>('draft-b');
...
Baseline: ${COMPARISON_DEMO_DATA.docA.name}
Counter-Offer: ${COMPARISON_DEMO_DATA.docB.name}

AI EXECUTIVE SUMMARY:
${COMPARISON_DEMO_DATA.summaryAI}

KEY CLAUSE VARIATIONS:
${COMPARISON_DEMO_DATA.comparisonItems
  .map(
    (item) =>
      `[${item.category}] ${item.field}\n  • Draft A: ${item.docAValue}\n  • Draft B: ${item.docBValue}\n  • AI Assessment: ${item.aiExplanation}`
  )
  .join('\n\n')}

Disclaimer: Generated by NyayaLens for informational comparison and preparation. Does not replace advice from a qualified legal professional.`;
  ...
  const riskCount = COMPARISON_DEMO_DATA.comparisonItems.filter((i) => i.differenceType === 'warning').length;
  const keyDiffCount = COMPARISON_DEMO_DATA.comparisonItems.filter((i) => i.differenceType === 'significant_difference').length;
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-xl bg-white border border-slate-200 shadow-2xs">
          ...
          <strong className="text-sm font-extrabold text-slate-900 block mt-0.5">
            {COMPARISON_DEMO_DATA.comparisonItems.length} Key Clauses
          </strong>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-emerald-50/70 border border-emerald-200 shadow-2xs">
          ...
          <strong className="text-sm font-extrabold text-emerald-800 block mt-0.5">
            Deposit & Entry Notice
          </strong>
        </div>
      </div>

      {/* 4. AI Comparative Executive Synthesis */}
