...
function AskDocumentContent() {
  ...
  return (
    <div className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex flex-col h-[calc(100vh-8rem)] animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-slate-200 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            aria-label="View full document in analysis workspace"
          >
      ...
      <div role="group" aria-label="Suggested questions" className="py-3 shrink-0 flex items-center gap-2 overflow-x-auto">
        ...
        {suggestedQuestions.map((q, i) => (
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-full bg-white border border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/50 text-slate-700 hover:text-indigo-900 transition whitespace-nowrap shadow-2xs focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
            {q}
          </button>
        ))}
      ...
      <div
        ...
        aria-live="polite"
        className="flex-1 bg-white rounded-2xl border border-slate-200 p-5 overflow-y-auto space-y-5 shadow-subtle my-2"
      >
        {messages.map((msg) => (
          <div
            key={msg.id}
            ...
            >
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between font-mono font-semibold text-slate-900">
                          ...
                          <span className="text-[11px] text-slate-600 font-medium">Page {src.page}</span>
                        </div>
                        <p className="text-[11px] text-slate-700 font-mono italic leading-relaxed bg-slate-50/80 p-2 rounded border border-slate-100">
                          "{src.snippet}"
                        </p>
                        <div className="pt-1 flex justify-end">
                          <Link
                            href={`/analyze?clause=${clauseId}`}
                            aria-label={`Jump to ${src.section} ${src.clauseTitle} in document viewer`}
                            className="inline-flex items-center gap-1 text-[11px] font-semibold text-indigo-700 hover:text-indigo-900 transition"
                          >
              ...
              <div className="flex items-center justify-between pt-1 text-[10px] text-slate-600">
                <button
                  ...
                  className="hover:text-slate-900 inline-flex items-center gap-1 opacity-90 hover:opacity-100 transition focus:outline-none focus:ring-1 focus:ring-slate-400 rounded px-1"
                  title="Copy answer"
                  aria-label="Copy answer and verified citations to clipboard"
                >
                  {copiedId === msg.id ? (
                    <>
                ...
                <span className="font-mono text-slate-600 font-medium">{msg.timestamp}</span>
              </div>
            </div>
          </div>
        ))}
        ...
        {isLoading && (
          <div role="status" aria-live="polite" className="flex gap-3 mr-auto max-w-2xl">
            <div className="w-8 h-8 rounded-xl bg-indigo-50 border border-indigo-200 text-indigo-700 flex items-center justify-center shrink-0" aria-hidden="true">
              <Bot className="w-4 h-4" />
            </div>
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs text-slate-700 font-medium flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-indigo-600 animate-spin" aria-hidden="true" />
      ...
      >
        <div className="flex items-center gap-2 bg-white rounded-2xl border border-slate-300 focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-500/20 p-2 shadow-subtle">
          <label htmlFor="ask-question-input" className="sr-only">
            Ask a question about {currentDocument.name}
          </label>
          <input
            id="ask-question-input"
            type="text"
            placeholder={`Ask a question about ${currentDocument.name}...`}
            aria-label={`Ask a question about ${currentDocument.name}`}
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            disabled={isLoading}
            className="flex-1 bg-transparent px-3 py-2 text-xs sm:text-sm text-slate-900 placeholder:text-slate-500 focus:outline-none"
          />
          ...
          <button
            ...
            aria-label={isListening ? 'Stop voice dictation' : 'Dictate question with microphone'}
            className={cn(
              'p-2.5 rounded-xl transition focus:outline-none focus:ring-2 focus:ring-indigo-500',
              isListening
          ...
          <button
            ...
            className="p-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 disabled:opacity-40 text-white transition shadow-2xs focus:outline-none focus:ring-2 focus:ring-slate-900"
